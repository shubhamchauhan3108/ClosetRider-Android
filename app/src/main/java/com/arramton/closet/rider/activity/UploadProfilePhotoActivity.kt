...
class UploadProfilePhotoActivity : AppCompatActivity() {
    ...
    fun init(){
        ...
        profileRegisterBtb.setOnClickListener {
            if (profileUrl.toString().trim().isEmpty()){
                Toast.makeText(this@UploadProfilePhotoActivity,"Please Upload Photo",Toast.LENGTH_SHORT).show()
        ...
        authViewModel.registerObservable.observe(this, Observer {
            if(it!=null){
                if (it.success){
                    Toast.makeText(this@UploadProfilePhotoActivity,it.message,Toast.LENGTH_SHORT).show()
                    startActivity(Intent(this@UploadProfilePhotoActivity,LoginActivity::class.java))
                    finishAffinity()
                }
            }
        })
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == pic_id) {
            ...
            val path: String = MediaStore.Images.Media.insertImage(this@UploadProfilePhotoActivity.getContentResolver(), photo, "image", null)
...
//
    ...
    fun register(){
        ...
        authViewModel.registerObserver(name,mobile,email,gender,address,landMark,state,city,pincode,adharNumber,front,back,panNumber,pancard,bankName,accountHolderName,accountNumber,IFSCCode,profileUrl)
    }
}
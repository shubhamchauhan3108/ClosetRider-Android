    ...
    fun init(){
        ...
        registerMaterialBtn.setOnClickListener {
            ...
            if (etMobile.text.toString().trim().isEmpty()){
                Toast.makeText(this@RegisterActivity,"Enter Mobile Number",Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            ...
            if (etGender.text.toString().trim().isEmpty()){
            ...
            if (etLandMark.text.toString().trim().isEmpty()){
                Toast.makeText(this@RegisterActivity,"Enter Landmark",Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (stateId==0){
                Toast.makeText(this@RegisterActivity,"Please Select State ",Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (cityId==0){
                Toast.makeText(this@RegisterActivity,"Please Select City ",Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            ...
            val intent:Intent= Intent(this@RegisterActivity,UploadDocumentsActivity::class.java)
...
//        stateList=
        ...
        val gson = Gson()
        val state:StateModelResponse = gson.fromJson(readJsonFile(this@RegisterActivity,"india_states.json"), StateModelResponse::class.java)

        stateList=state.states

        val stateAdapter:StateAdapter=StateAdapter(this@RegisterActivity,stateList,object :StateListener{
...
//        val city:StateModelResponse = gson.fromJson(readJsonFile(this@RegisterActivity,"india_states.json"), StateModelResponse::class.java)
        ...
        val cityAdapter:CityAdapter= CityAdapter(this@RegisterActivity,cityList,object :StateListener{
            override fun stateName(id:Int,name: String) {
                tvCity.text=name
                cityId=id
                dialog.dismiss()

...
class OrderDetailsChildCategoryAdapter(val context: Context,val childList:List<CostumesOrderItem>) : RecyclerView.Adapter<OrderDetailsChildCategoryAdapter.ViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): OrderDetailsChildCategoryAdapter.ViewHolder {
       val view:View=LayoutInflater.from(parent.context).inflate(R.layout.custom_order_details_child_layout,parent,false)
        return ViewHolder(view)
    }

    override fun onBindViewHolder(holder: OrderDetailsChildCategoryAdapter.ViewHolder, position: Int) {
...
//            pickupRequestListener.listSize(""+dataList.get(position).getCostumes().size());
        }
        ...
        holder.showLayout.setOnClickListener {
            ...
            val orderDetailsSubChlidAdapter:OrderDetailsSubChlidAdapter= OrderDetailsSubChlidAdapter(context,childList.get(position).order_item)
